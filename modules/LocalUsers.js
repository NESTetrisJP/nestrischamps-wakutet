...
const _importUsers = async (
	...
	console.log(
		`Response hash computed in ${(performance.now() - start).toFixed(
			4
		)}ms: ${hash}`
	);
	...
	console.log(
		`Parsed ${players.length} players in ${(performance.now() - start).toFixed(
			4
		)}ms`
	);
	...
	console.log(
		`DONE - Inserted ${players.length} players in ${(
			performance.now() - init
		).toFixed(4)}ms`
	);
